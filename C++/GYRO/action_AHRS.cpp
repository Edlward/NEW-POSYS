...
static action_matrix P_mag(4, 4, MATRIX_I);
static action_matrix P_acc(4, 4, MATRIX_I);
void AHRS_Init()
{
	Q.set_data(0, 0, 0.1f);
	Q.set_data(1, 1, 0.1f);
	...
	R_mag.set_data(0, 0, 0.1f);
	R_mag.set_data(1, 1, 0.1f);
	R_mag.set_data(2, 2, 0.1f);
	
	R_acc.set_data(0, 0, 1.0f);
	R_acc.set_data(1, 1, 1.0f);
	R_acc.set_data(2, 2, 1.0f);
	
	P_mag.set_data(0,0,0.001f);
	P_mag.set_data(1,1,0.001f);
	P_mag.set_data(2,2,0.001f);
...
//    test.PrintfItself();
		...
		P_mag = (I - K*H_mag)*P_pre*!(I - K*H_mag) + K*R_mag*!K;
		//P_mag.PrintfItself();
		...
		P_acc = (I - K*H_acc)*P_acc*!(I - K*H_acc) + K*R_acc*!K;
