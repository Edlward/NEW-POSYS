		...
		if(temp>(TempTable_min-1)&&temp<(TempTable_max+1))
		{
				 ...
				 oldCountNum[i]!=0xffffffff)
			{
				...
				*/
				...
				{
					//后三个参数分别是y,x,z的零飘角速度
					ordata_input(temp,(*(oldResult+i)),
					                  (*(oldResult+i+10*(TempTable_max-TempTable_min))),
					                  (*(oldResult+i+20*(TempTable_max-TempTable_min))));
				}
			}
		}
		temp=temp+0.1f;
